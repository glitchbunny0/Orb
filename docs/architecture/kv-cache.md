...
`prefix` + the writer's trailing user message are both reused. Only the writer's draft and the editor instructions are new. The writer's prose is typically a few hundred tokens; the cached prefix is many thousands.
